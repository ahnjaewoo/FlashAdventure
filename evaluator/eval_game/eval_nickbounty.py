...
async def eval_nickbounty():
    game = "Nick Bounty: A Case of the Crabs"
    ...
    if affection is not None:
        ...
        result_obj = {
            "game": "nick_bounty_a_case_of_the_crabs",
            "result": f"Found Place: {affection}",
            "found_place": affection
        }

        Path("results").mkdir(exist_ok=True)
        with open("results/result_nickbounty.json", "w") as f:
            json.dump(result_obj, f, indent=2)
        print("📝 Result saved to results/result_nickbounty.json")
    else:
        print("⚠️ Couldn't find affection score in result.")

if __name__ == "__main__":
    asyncio.run(eval_nickbounty())