...
async def eval_festival():
    game = "Festival Days Sim Date"
    ...
    if affection is not None:
        ...
        result_obj = {
            "game": "festival_sim_date",
            "result": f"EXP: {affection}",
            "EXP": affection
        }

        Path("results").mkdir(exist_ok=True)
        with open("results/result_festival.json", "w") as f:
            json.dump(result_obj, f, indent=2)
        print("📝 Result saved to results/result_festival.json")
    else:
        print("⚠️ Couldn't find affection score in result.")

if __name__ == "__main__":
    asyncio.run(eval_festival())