...
async def eval_grim2():
    ...
    if affection is not None:
        ...
        result_obj = {
            "game": "grim_tales_the_legacy_collectors_edition",
            "result": f"Items: {affection}",
            "Items": affection
        }

        Path("results").mkdir(exist_ok=True)
        with open("results/result_grim2.json", "w") as f:
            json.dump(result_obj, f, indent=2)
        print("📝 Result saved to results/result_grim2.json")
    else:
        print("⚠️ Couldn't find Found Items in result.")

if __name__ == "__main__":
    asyncio.run(eval_grim2())