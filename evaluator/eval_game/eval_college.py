...
def extract_affection(conversation: List[str]) -> float | None:
    for msg in reversed(conversation):
        match = re.search(r"Combined Score:\s*(\d+(\.\d+)?)", msg, re.IGNORECASE)
        if match:
            return float(match.group(1))
    return None

async def eval_college():
    game = "Community College Sim"
    ...
    if affection is not None:
        print(f"Combined Score: {affection}")

        result_obj = {
            "game": "college_sim",
            "result": f"Combined Score: {affection}",
            "Combined Score": affection
        }

        Path("results").mkdir(exist_ok=True)
        with open("results/result_college.json", "w") as f:
            json.dump(result_obj, f, indent=2)
        print("📝 Result saved to results/result_college.json")
    else:
        print("⚠️ Couldn't find Combined Score in result.")

if __name__ == "__main__":
    asyncio.run(eval_college())