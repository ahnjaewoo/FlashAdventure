...
def extract_affection(conversation: List[str]) -> float | None:
    for msg in reversed(conversation):
        match = re.search(r"Affection:\s*(\d+(\.\d+)?)", msg, re.IGNORECASE)
        if match:
...
async def eval_pico():
    ...
    if affection is not None:
        print(f"💖 Affection Score: {affection}")

        result_obj = {
            "game": "pico_sim_date",
            "result": f"Affection: {affection}",
            "affection": affection
        }

        Path("results").mkdir(exist_ok=True)
        with open("results/result_pico.json", "w") as f:
            json.dump(result_obj, f, indent=2)
        print("📝 Result saved to results/result_pico.json")
    else:
        print("⚠️ Couldn't find affection score in result.")

if __name__ == "__main__":
    asyncio.run(eval_pico())