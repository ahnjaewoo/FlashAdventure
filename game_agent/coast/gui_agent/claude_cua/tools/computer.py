...
class PyAutoGUIComputerTool:
    ...
    _action_count: int = 0
    _max_actions: int = 100  # Optional limit

    def increment_action_count(self, action: str):
        if self.should_count_action(action):
            self._action_count += 1
            print(f"[ACTION COUNT] {self._action_count}/{self._max_actions}")
            if self._action_count >= self._max_actions:
                print("[LIMIT REACHED] Maximum actions reached.")
