...
class head():
    ...
    def planning_actions(self):
        if self.reasoning_model == "anthropic":
            planning_result = plan_actions(self.task_prompt, self.reasoning_model, "claude-3-7-sonnet-20250219", self.gui_model, self.game_name, self.env_summary, self.before_image, self.width)
        elif self.reasoning_model == "openai":
            planning_result = plan_actions(self.task_prompt, self.reasoning_model, "gpt-4o", self.gui_model, self.game_name, self.env_summary, self.before_image, self.width)
        else:
            ValueError("planning_actions: This model is not working")
