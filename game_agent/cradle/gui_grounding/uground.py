...
def execute_gui_action(action: Dict[str, Any], computer: LocalDesktopComputer):
    ...
    match action_type:
        ...
        case _:
            print(f"⚠️ Unsupported action type (ignored): {action_type}")
            return


# --- Planning (retry up to 3 times including API errors) ---
...
async def agent_step(
    ...
    # 2. If grounding is required
    if plan["type"] in ["click", "double_click", "drag", "scroll"]:
        client_uground = AsyncOpenAI(api_key="empty", base_url="http://seurat.snu.vision:8000/v1")
        x, y = await ground_with_uground(plan["description"], encoded_image, client_uground)
        plan["x"] = x
        plan["y"] = y

    # 3. Execute (ignore errors)
    try:
        execute_gui_action(plan, computer)
    except Exception as e:
        print(f"⚠️ Error during action execution (ignored, continuing): {e}")
